# OpenLock v3 — optional plaintext TOTP scheme

This is the implemented wire contract for the optional TOTP scheme in the
[v3 family](protocol.md). `MUST` denotes a verifier requirement. The secure v2
scheme remains fully supported by its separate implementation; this TOTP parser
rejects its messages and never falls back to them. Only unlocking is an
authenticated access operation in the TOTP scheme.
...
V2 codes not used by this scheme remain reserved in its response format. Unknown result codes are rejected. The C ABI
